...
const menuItems = [
  { category: 'Drinks', items: [
    ...
    { name: 'ﾌﾚｰﾊﾞｰﾃｨｰ', image: '/images/flavor.jpg', description: '・レモンティー\n・はちみつ\n' },
...
const DayComponent: FC = () => (
  <ThemeProvider theme={dayTheme}>
    <div className={kiwiMaru.className}>
      ...
      >
        {menuItems.map((menu) => (
          <Accordion key={menu.category}  sx={{ backgroundColor: dayTheme.palette.background.default }}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}> 
              <Typography variant="h4">{menu.category}</Typography>
            </AccordionSummary>
            <AccordionDetails sx={{ backgroundColor: dayTheme.palette.background.default }}>
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2, justifyContent: 'center', maxWidth: '1000px', margin: '0 auto' }}>
                {menu.items.map((item) => (
